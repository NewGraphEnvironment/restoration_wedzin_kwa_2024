...
Table \@ref(tab:tab-area-priority-cap) integrates land cover change, fish habitat modelling, land ownership, and cultural site data to characterize each sub-basin. Below Bulkley Falls, the middle reaches show the steepest declines — Bulkley Byman-JDavid (-14.5%) and Bulkley Houston-McKilligan (-11.1%) — while the reach closest to Houston (Bulkley-Houston, -8.3%) is comparatively stable but contains the most coho spawning habitat (40 km). The Bulkley Richfield-Falls sub-basin has the largest modelled floodplain below the falls (1,060 ha), the highest coho spawning length (60.8 km), and a near-even split of private (648 ha) and non-private (564 ha) floodplain — existing restoration investments here provide a foundation for expansion. Above Bulkley Falls, Maxam Creek stands out with 59.4% agriculture expansion, 97 ha of reserve land, and a documented traditional fishing site. Buck Creek Downstream Falls has 2 cultural sites and the highest chinook spawning density (60.5 km). These data provide the empirical foundation for the prioritization framework described in [Recommendations](#recommendations), where governance structure and community input guide how investment is directed across sub-basins.

```{r tab-area-priority-cap, results="asis"}
my_caption <- "Sub-basin prioritization data for the Neexdzii Kwah watershed. Floodplain tree loss and agriculture change are from drift land cover classification (2017-2023). Fish habitat lengths are modelled spawning and rearing from bcfishpass. Non-private floodplain ownership includes Crown Provincial, Untitled Provincial, Crown Agency, Local Government, Federal, and Unclassified parcels from the PMBC parcel fabric. Reserves are from CLAB and cultural sites from @gottesfeld_rabnett2007SkeenaFish. Sub-basins are ordered by tree loss rate within the floodplain."
...
area_scores <- readr::read_csv(
  ...
  dplyr::arrange(tree_loss_pct) |>
  dplyr::mutate(
    dplyr::across(c(area_km2, floodplain_area_ha), ~ round(.x, 0)),
    dplyr::across(c(fp_private_ha, fp_crown_ha), ~ round(.x, 0)),
    position = gsub("_", " ", gsub("below_|above_", "", position))
  ) |>
  dplyr::mutate(fisheries_value = NA_character_) |>
  dplyr::select(
    `Sub-basin` = name_basin,
    `Area (km²)` = area_km2,
    `Floodplain (ha)` = floodplain_area_ha,
    `Tree Loss (%)` = tree_loss_pct,
    `Ag Change (%)` = ag_change_pct,
    ...
    `Reserves (ha)` = reserve_area_ha,
    `Fisheries Value` = fisheries_value,
    `Cultural Sites` = n_cultural_sites
  )

my_dt_table(area_scores, page_length = 14, cols_freeze_left = 0)
```
