...
Land cover classification within the modelled floodplain (IO LULC 10 m, Sentinel-2 derived) shows a consistent pattern of tree cover loss and agriculture expansion between 2017 and 2023. Across all sub-basins, approximately `r total_tree_loss` ha of tree cover was lost within the floodplain, with corresponding gains in the Agriculture superclass (Crops + Rangeland + Bare Ground). The greatest losses occurred in `r top3$subbasin[1]` (`r round(abs(top3$delta_ha[1]), 0)` ha), `r top3$subbasin[2]` (`r round(abs(top3$delta_ha[2]), 0)` ha), and `r top3$subbasin[3]` (`r round(abs(top3$delta_ha[3]), 0)` ha). Some pixel-level transitions reflect channel migration, seasonal differences in vegetation health due to drought, or classification noise rather than actual land clearing — particularly along actively shifting reaches and at field-forest edges. Despite these sources of uncertainty, the overall trajectory of tree cover loss and agriculture expansion is consistent across sub-basins and time steps, and the magnitude of change is well above what classification error alone would produce. These trends highlight where protection of remaining floodplain tree cover, engagement with landowners, and restoration of degraded reaches may be most effective. An interactive map of classified land cover with transition overlays, along with detailed sub-basin summaries, is presented in [Appendix - LULC](#app-lulc).
...
Trend statistics, spatial anomaly maps, and time series plots are presented in [Appendix 1 - Climate Anomaly Data](#app-climate-anomaly).
