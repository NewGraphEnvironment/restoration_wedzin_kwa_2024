...
floodplain <- sf::st_read(fp_file, layer = "co_ff04", quiet = TRUE) |> sf::st_transform(4326)
...
Land cover change is assessed within the modelled floodplain — the area delineated by the Valley Confinement Algorithm [VCA; @nagel_etal2014LandscapeScale, `flooded` package] for coho rearing/spawning habitat on streams of 3rd order and greater, with lakes and wetlands included. The VCA uses a bankfull depth regression [@hall_etal2007Predictingriver] scaled by `flood_factor = 4` to approximate the functional floodplain — the area regularly influenced by high flows that sustains riparian vegetation, off-channel habitat, and large wood recruitment processes critical to salmon. The flood factor of 4 was selected as a conservative estimate between the empirically validated historical floodplain extent [flood factor = 3 on 10 m DEM; @hall_etal2007Predictingriver] and the recommended range of 5-7 for valley bottom delineation [@nagel_etal2014LandscapeScale], accounting for the coarser vertical resolution of the 25 m DEM used here. The modelled floodplain covers `r round(as.numeric(sf::st_area(sf::st_union(floodplain))) / 1e6, 1)` km² along the Neexdzii Kwa (Bulkley River) and tributaries. All percentages and areas reported below refer to land cover within this modelled floodplain, not the full watershed.
...
```{r plot-lulc-ag-stacked, fig.cap="Composition of the Agriculture superclass within each sub-basin's modelled floodplain. Rangeland dominates; Crops and Bare Ground are minor components.", fig.width=12, fig.height=8}
lulc_all |>
