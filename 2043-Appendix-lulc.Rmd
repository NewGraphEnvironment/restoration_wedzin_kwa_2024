...
floodplain <- sf::st_read(
  file.path(lulc_dir, "floodplain_neexdzii_co.gpkg"), quiet = TRUE
) |> sf::st_transform(4326)

# Label sub-basins (matches lulc_classify.R)
subbasins <- subbasins |>
...
Land cover change is assessed within the modelled floodplain — the valley bottom area delineated by the Valley Confinement Algorithm (`flooded` package) for co rearing/spawning habitat on streams of 4th order and greater. The modelled floodplain covers `r round(as.numeric(sf::st_area(sf::st_union(floodplain))) / 1e6, 1)` km² along the Neexdzii Kwa (Bulkley River) and tributaries. All percentages and areas reported below refer to land cover within this modelled floodplain, not the full watershed.
...
# Load transition raster and build summary
...
# Add sub-basin outlines as toggleable overlay
trans_groups <- transition$summary$transition[
  transition$summary$from_class != transition$summary$to_class
]
...
m <- m |>
  ...
  leaflet::addPolygons(
    data = subbasins, fill = FALSE, color = "#5C4033",
...
```{r map-lulc-subbasins, fig.cap="Sub-basin boundaries and modelled floodplain extent used for land cover analysis.", fig.width=8.5, fig.height=11}
